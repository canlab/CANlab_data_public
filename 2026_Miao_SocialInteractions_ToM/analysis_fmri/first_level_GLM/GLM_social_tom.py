## first-level GLM analysis for theory of mind (ToM) and social interactions with participants' moment-by-moment ratings 
# Author: Zizhuang Miao

# Main regressors:
# rating: the rating period in every trial (the last 8 seconds)
# ToM: coded as a float from -1 to 1, representing the median of online participants' ratings of how much they were using ToM at each moment (-1 is not at all and 1 is definitely yes)
# SInt: coded as a float from -1 to 1, representing the median of online participants' ratings of social interactions at each moment (-1 is not at all and 1 is definitely yes)
# NOTE: both regressors are mean-centered for story periods for each run, such that 0 corresponds to the mean of ToM or SInt ratings in the story period of each run
...
tomEventsDir = 'data/all_design_matrices/designs_tom'    # events files for theory of mind ratings generated from behavioral data
socEventsDir = 'data/all_design_matrices/designs_social'
outputDir = ''
...
subList = pd.read_csv('cleanedSubjects.csv')['subjects']  # rerun some subjects
